...
class VeoVideoService:
    ...
    def _start(self, model: str, prompt: str, aspect_ratio: str) -> tuple[str | None, str | None]:
        ...
        if r.status_code != 200:
            return None, f"Veo start HTTP {r.status_code}: {r.text[:300]}"
        name = r.json().get("name")
        if not name:
            return None, "Veo start returned no operation name."
        return name, None
    ...
    def _poll(self, op_name: str) -> tuple[bytes, str, str | None]:
        ...
        while time.monotonic() < deadline:
            ...
            if r.status_code != 200:
                return b"", "error", f"Veo poll HTTP {r.status_code}: {r.text[:300]}"
            body = r.json()
            if body.get("done"):
                if body.get("error"):
                    return b"", "error", f"Veo error: {str(body['error'])[:300]}"
